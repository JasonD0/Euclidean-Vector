...
SCENARIO("Constructors") {
  ...
  GIVEN("an integer x and a double y") {
    ...
    WHEN("a EuclideanVector is constructed using y") {
      EuclideanVector ev{x, y};
    }
  }

  GIVEN("a vector") {
    std::vector<double> v{2.3, 5.4, 1.0, 0.4};
...
SCENARIO("Assignments") {
  GIVEN("two EuclideanVectors ev1 & ev2") {
    ...
    WHEN("ev1 is copy assigned to ev2") {
      ...
      THEN("ev1 contains the same values as ev2") {
        REQUIRE(ev1 == ev2);
      }
    }
    ...
    WHEN("ev2 is move assigned to ev1") {
      ...
      THEN("ev2 contains ev1's elements") {
        REQUIRE(ev2 == EuclideanVector{3, 9.53});
      }
    }
  }
}
...
SCENARIO("Using Subscript") {
  GIVEN("a EuclideanVector and an integer") {
    ...
    WHEN("getting the value at the integer using subscript") {
      ...
      THEN("the value at that index is returned") {
        REQUIRE(std::fabs(res - 9.5) < 2.22045e-016);
      }
    }
    ...
    WHEN("setting the value at the integer using subscript") {
      ...
      THEN("the value at that index is changed") {
        REQUIRE(ev[i] == 9.99);
      }
    }
  }
...
SCENARIO("EuclideanVector Methods") {
  GIVEN("a EuclideanVector with different values") {
    ...
    WHEN("getting the Euclidean Norm") {
      ...
      THEN("the euclidean norm is returned") {
        REQUIRE(std::fabs(norm - 7.0) < 2.22045e-016);
      }
    }
    ...
    WHEN("getting its unit vector") {
      ...
      THEN("the unit vector is returned") {
        REQUIRE(uv == correct_uv);
      }
    }
  }
}
